...
HdlStatus AtomBombingMethod(uint32_t pid, const wchar_t* dll_path, uint64_t* out_base) {
    ...
    for (size_t ti = tids.size(); ti-- > 0;) {
        HANDLE thread =
            OpenThread(THREAD_SET_CONTEXT | THREAD_QUERY_INFORMATION | 0x0004 /*THREAD_ALERT*/, FALSE,
                       tids[ti]);
        if (!thread) {
            thread = OpenThread(THREAD_SET_CONTEXT | THREAD_QUERY_INFORMATION, FALSE, tids[ti]);
        }
        if (!thread) {
            continue;
        }
        ...
        if (!path_mem.Alloc(process, path_bytes, PAGE_READWRITE) ||
            !stub_mem.Alloc(process, sizeof(X64LoadLibraryStub), PAGE_EXECUTE_READWRITE)) {
            ReleaseAtoms(atoms);
            CloseHandle(thread);
            continue;
        }
        ...
        if (QueueAtomPathWrites(thread, path_mem.ptr, atoms, chunk_lens) != HDL_OK ||
            ApcMemsetWrite(thread, stub_mem.ptr, &stub, sizeof(stub)) != HDL_OK) {
            ReleaseAtoms(atoms);
            CloseHandle(thread);
            continue;
        }

        NTSTATUS nt = nt_q(thread, stub_mem.ptr, nullptr, nullptr, nullptr);
        if (nt < 0) {
            nt = nt_q(thread, reinterpret_cast<PVOID>(load_library), path_mem.ptr, nullptr, nullptr);
        }
        if (nt < 0) {
            ReleaseAtoms(atoms);
            CloseHandle(thread);
