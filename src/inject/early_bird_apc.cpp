...
HdlStatus EarlyBirdApcMethod(const wchar_t* exe_path, const wchar_t* dll_path, uint32_t* out_pid,
    ...
    if (!CreateProcessW(exe_path, cmd_buf.data(), nullptr, nullptr, FALSE, CREATE_SUSPENDED, nullptr,
                        nullptr, &si, &pi)) {
    ...
    RemoteAlloc remote;
    HdlStatus st = WriteRemotePath(pi.hProcess, dll_path, remote);
    if (st != HDL_OK) {
    ...
    if (!QueueUserAPC(load_library, pi.hThread, reinterpret_cast<ULONG_PTR>(remote.ptr))) {
