...
size_t ProbeTableEntries(HANDLE process, uint64_t table_ptr) {
    ...
    const size_t bytes =
        reinterpret_cast<uint8_t*>(mbi.BaseAddress) + mbi.RegionSize - reinterpret_cast<uint8_t*>(table_ptr);
...
HdlStatus KernelCallbackTableMethod(uint32_t pid, const wchar_t* dll_path, uint64_t* out_base) {
    ...
    ULONG ret_len = 0;
    NTSTATUS nt =
        nt_query(process, ProcessBasicInformation, &pbi, sizeof(pbi), &ret_len);
    ...
    if (!ReadRemote(process, peb + kPebKernelCallbackTable, &table_ptr, sizeof(table_ptr)) ||
        table_ptr == 0) {
        CloseHandle(process);
    ...
    const size_t entries = ProbeTableEntries(process, table_ptr);
    std::vector<uint64_t> table(entries, 0);
    if (!ReadRemote(process, reinterpret_cast<void*>(table_ptr), table.data(),
    ...
    if (!new_table.Alloc(process, entries * sizeof(uint64_t)) ||
    ...
    if (!WriteRemote(process, peb + kPebKernelCallbackTable, &new_table_addr, sizeof(new_table_addr))) {
        CloseHandle(process);
    ...
    if (!payload_mem.Alloc(process, sizeof(payload)) ||
        !payload_mem.Write(payload, sizeof(payload))) {
        WriteRemote(process, peb + kPebKernelCallbackTable, &table_ptr, sizeof(table_ptr));
        CloseHandle(process);
        return HDL_E_NO_MEM;
    }
    ...
    RemoteAlloc cds_mem;
    if (!cds_mem.Alloc(process, sizeof(cds)) || !cds_mem.Write(&cds, sizeof(cds))) {
        WriteRemote(process, peb + kPebKernelCallbackTable, &table_ptr, sizeof(table_ptr));
        CloseHandle(process);
        return HDL_E_NO_MEM;
    }
    ...
    if (!send_stub_mem.Alloc(process, sizeof(send_stub), PAGE_EXECUTE_READWRITE) ||
    ...
    if (!send_args_mem.Alloc(process, sizeof(send_args)) ||
    ...
    HANDLE t = ::CreateRemoteThread(process, nullptr, 0,
                                    reinterpret_cast<LPTHREAD_START_ROUTINE>(send_stub_mem.ptr),
