...
// LoadLibraryW(path) then NtContinue(saved_context) so RSP/RIP/regs match the pre-call thread state.
// This avoids brittle RtlRemoteCall post-return stack restore across Windows builds.
#pragma pack(push, 1)
...
HdlStatus RtlRemoteCallMethod(uint32_t pid, const wchar_t* dll_path, uint64_t* out_base) {
    ...
    bool invoked = false;
    for (DWORD tid : tids) {
        HANDLE thread = OpenThread(THREAD_SET_CONTEXT | THREAD_GET_CONTEXT | THREAD_SUSPEND_RESUME |
        ...
        RemoteAlloc ctx_mem;
        if (!ctx_mem.Alloc(process, sizeof(CONTEXT)) || !ctx_mem.Write(&ctx, sizeof(CONTEXT))) {
            ResumeThread(thread);
            CloseHandle(thread);
            continue;
        }
        ...
        NTSTATUS nt = rtl_remote(process, thread, stub_mem.ptr, 0, nullptr, FALSE, TRUE);
        if (nt < 0) {
            ...
            nt = rtl_remote(process, thread, reinterpret_cast<PVOID>(load_library), 1, args, FALSE,
                            TRUE);
        }
